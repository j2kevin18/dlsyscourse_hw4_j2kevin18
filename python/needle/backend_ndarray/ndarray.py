...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        old_shape_prod = np.array(list(self.shape)).prod()
        new_shape_prod = np.array(list(new_shape)).prod()

        if self.is_compact == False or new_shape_prod != old_shape_prod:
            raise ValueError("The product of current shape is not equal to the product of the new shape, or if the matrix is not compact")

        new_strides_pre =  list(np.array(list(new_shape)[:0:-1]).cumprod().astype(int)[::-1])
        new_strides_pre.append(1)
        new_strides = tuple(new_strides_pre)
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = tuple([math.ceil((slice_t.stop-slice_t.start)/slice_t.step) for slice_t in idxs])

        new_strides =  tuple([stride * slice_t.step for slice_t, stride in zip(idxs, self.strides)])

        offset = 0
        for slice_t, stride in zip(idxs, self.strides):
            offset += slice_t.start * stride

        # print(self.shape, self.strides, new_shape, new_strides, offset)

        array = self.make(new_shape, strides=new_strides, device=self.device, handle=self._handle, offset=offset)
        return array
        ### END YOUR SOLUTION
    ...
    def reduce_view_out(self, axis, keepdims=False):
        ...
        if axis is None:
            ...
            #我自己改的
            out = NDArray.make(
                tuple([1 for _ in range(self.ndim)])
                if keepdims else
                (1,),
                device=self.device,
            )
    ...
    def flip(self, axes):
        ...
        shape_list.append(1)
        shape_prob_list = np.array(list(reversed(shape_list))).cumprod()

        for i in range(self.ndim):
            if i in axes:
                index = self.ndim - i
                new_offset += np.array(shape_prob_list[index]).prod() - np.array(shape_prob_list[index-1]).prod()
                new_strides[i] *= -1

        array = NDArray.make(self.shape, strides=tuple(new_strides), device=self._device, handle=self._handle, offset=new_offset)
    ...
    def pad(self, axes):
        ...
        for _, (cur_dim_size, (pad_before, pad_after)) in enumerate(zip(old_shape, list(axes))):

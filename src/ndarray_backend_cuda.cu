...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN YOUR SOLUTION
  if(gid < size) out[gid] = a[offset+getIndex(shape, gid, strides)];
  /// END YOUR SOLUTION
}
...
__global__ void EwiseSetitemKernel(size_t size, const scalar_t* a, scalar_t* out, CudaVec shape,
                  CudaVec strides, size_t offset) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if(gid < size) out[offset+getIndex(shape, gid, strides)] = a[gid];
}
...
__global__ void ScalarSetitemKernel(size_t size, scalar_t val, scalar_t* out, CudaVec shape,
                  CudaVec strides, size_t offset) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if(gid < size) out[offset+getIndex(shape, gid, strides)] = val;
}
...
__global__ void MatmulTileKernel(scalar_t* a, scalar_t* b, scalar_t* out, uint32_t M, uint32_t N,
            uint32_t P) {
      ...
      if(tidr + bidr < M && tidc + bidc < P) out[P * (tidr + bidr) + tidc + bidc] = Outvalue;
}
...
__global__ void ReduceMaxKernel(scalar_t* a, scalar_t* out, size_t out_size, size_t reduce_size) {
      ...
      // printf("enter up reduce function.\n");
